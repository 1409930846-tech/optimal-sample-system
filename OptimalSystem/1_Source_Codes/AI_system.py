...
1. Parameter input with validation (m, n, k, j, s, t)
...
class OptimalSamplesSelectionSystem:
    ...
    def _create_parameter_input_frame(self) -> None:
        ...
        # Add t parameter with special handling (dynamic max based on C(j,s))
        self._create_t_param_group(param_groups_container)
    ...
    def _calculate_combination(self, n: int, r: int) -> int:
        ...
        if r == 0 or r == n:
            return 1
        # Use iterative formula to avoid large factorials
        r = min(r, n - r)
        result = 1
        for i in range(r):
    ...
    def _get_max_t(self) -> int:
        """Calculate the maximum value for t based on current j and s values."""
        try:
            j = int(self.param_entries['j'].get())
            s = int(self.param_entries['s'].get())
            return self._calculate_combination(j, s)
        except ValueError:
            return 1

    def _update_t_hint(self) -> None:
        """Update t parameter based on current j and s values."""
        try:
            j = int(self.param_entries['j'].get())
            s = int(self.param_entries['s'].get())

            # Special case: when s == j, C(j,s) = 1
            if s == j:
                # Lock t to 1
                self.param_entries['t'].config(state='normal')
                self.param_entries['t'].delete(0, tk.END)
                self.param_entries['t'].insert(0, "1")
                self.param_entries['t'].config(state='disabled')
        except ValueError:
            pass

    def _on_t_change(self, event=None) -> None:
        """Handle t parameter change events."""
        self._update_t_hint()

    def _on_j_s_change(self, event=None) -> None:
        """Handle j or s parameter change events - update t constraints."""
        self._validate_j_param()
        self._on_t_change()

    def _create_t_param_group(self, parent: tk.Widget) -> None:
        """Create the t parameter input group with validation hint."""
        group_frame = tk.Frame(parent, bg=self.COLOR_CARD)
        group_frame.pack(side=tk.LEFT, padx=15, pady=5)

        label = tk.Label(
            group_frame,
            text="At least t s-subsets per j-subset:",
            font=("Arial", 11),
            bg=self.COLOR_CARD,
            fg=self.COLOR_TEXT_DARK,
            anchor=tk.W
        ...
        self.param_entries['t'] = entry

        # Bind events for t parameter
        entry.bind('<FocusOut>', self._on_t_change)
        entry.bind('<KeyRelease>', self._on_t_change)

        # Bind j and s changes to update t constraints
        self.param_entries['j'].bind('<FocusOut>', self._on_j_s_change)
        self.param_entries['j'].bind('<KeyRelease>', self._on_j_s_change)
        self.param_entries['s'].bind('<FocusOut>', self._on_j_s_change)
        self.param_entries['s'].bind('<KeyRelease>', self._on_j_s_change)

    def _bind_validation_events(self) -> None:
        """Bind validation events to parameter entry fields."""
        # j validation is now handled in _create_t_param_group via _on_j_s_change
    ...
    def _validate_t_param(self) -> Optional[int]:
        """Validate the t parameter dynamically against current j and s values."""
        entry = self.param_entries['t']
        t_value = entry.get().strip()

        # If t is disabled (s == j case), return 1 directly
        ...
        try:
            ...
            messagebox.showerror(
                "Validation Error",
                "Parameter t must be a positive integer.\nPlease enter 1, 2, 3...",
                parent=self.root,
            )
            entry.delete(0, tk.END)
            entry.focus()
        ...
        max_t = self._get_max_t()
        if t > max_t:
            messagebox.showerror(
                "Validation Error",
                f"Parameter t cannot exceed C(j,s) = {max_t}.\n"
                f"Current valid range: 1 ≤ t ≤ {max_t}",
                parent=self.root,
            )
            entry.delete(0, tk.END)
            entry.focus()
    ...
    def _on_run_algorithm(self) -> None:
        ...
        try:
            results, info, filename = run_algorithm(
                m, n, k, j, s, t, self._selected_samples
            )
    ...
    def get_parameters(self) -> Dict[str, int]:
        ...
        # Add t parameter
        t_value = self.param_entries['t'].get()
        params['t'] = int(t_value) if t_value else 1
        return params
