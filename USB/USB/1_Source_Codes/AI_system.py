...
class OptimalSamplesSelectionSystem:
    ...
    def _create_file_operation_frame(self) -> None:
        ...
        # 右侧：操作按钮
        btn_frame = tk.Frame(file_inner_frame, bg=self.COLOR_CARD)
        ...
        # Run Algorithm 按钮单独一行
        self.run_algorithm_btn = ttk.Button(
            file_inner_frame,
            text="Run Optimal Grouping Algorithm",
            style='Blue.TButton',
            command=self._on_run_algorithm
        )
        self.run_algorithm_btn.pack(side=tk.BOTTOM, fill=tk.X, pady=(10, 0))
